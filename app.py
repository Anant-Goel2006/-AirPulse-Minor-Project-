...
import os, json, re, math, smtplib, ssl
import warnings
...
try:
    ...
    build_guidance_base = None
    generate_openai_guidance = None
    merge_guidance_payload = None
...
def resolve_waqi_token():
    ...
    invalid_tokens = {
        "",
        "your_waqi_api_token",
        "your_waqi_api_token_here",
        "your_waqi_token",
        "your_waqi_token_here",
        "replace-me",
        "changeme",
    }
...
SUBSCRIPTIONS_FILE = DATA_DIR / "aqi_alert_subscriptions.json"


def resolve_ipinfo_token():
    candidates = [
        ("IPINFO_TOKEN", os.getenv("IPINFO_TOKEN")),
        ("LOCATION_API_TOKEN", os.getenv("LOCATION_API_TOKEN")),
    ]
    invalid_tokens = {"", "your_ipinfo_token", "your_location_api_token", "replace-me", "changeme"}
    for source, raw in candidates:
        cleaned = str(raw or "").strip().strip('"').strip("'")
        if cleaned.lower() in invalid_tokens:
            continue
...
IPINFO_TOKEN, IPINFO_TOKEN_SOURCE = resolve_ipinfo_token()
IPINFO_BASE_URL = os.getenv("IPINFO_BASE_URL", "https://ipinfo.io").strip().rstrip("/")
FLASK_PORT    = int(os.getenv("FLASK_PORT",  8080))
FLASK_DEBUG   = os.getenv("FLASK_DEBUG", "False").lower() == "true"
APP_BUILD_TS  = int(time.time())
WAQI_TOKEN_MISSING_MSG = "WAQI token not configured on server"
SMTP_HOST = str(os.getenv("SMTP_HOST", "") or "").strip()
SMTP_PORT = int(str(os.getenv("SMTP_PORT", "587") or "587").strip() or "587")
SMTP_USER = str(os.getenv("SMTP_USER", "") or "").strip()
SMTP_PASSWORD = str(os.getenv("SMTP_PASSWORD", "") or "").strip()
SMTP_FROM_EMAIL = str(os.getenv("SMTP_FROM_EMAIL", SMTP_USER) or "").strip()
...
def waqi_token_missing_response(payload_shape="error"):
    if payload_shape == "status":
        return jsonify({
            "status": "error",
            "code": "waqi_token_missing",
            "data": WAQI_TOKEN_MISSING_MSG,
        }), 503
    return jsonify({
        "error": WAQI_TOKEN_MISSING_MSG,
        "code": "waqi_token_missing",
    }), 503
...
def normalize_lookup_key(raw):
    return normalize_query_text(raw).lower().strip()


def display_label_from_query(raw):
...
def normalize_feed_payload(payload):
    """Keep WAQI payload unchanged so the UI only uses official AQI values."""
    return payload
...
def enrich_location_guess(location, fallback=""):
    out = dict(location or {})
    city_key = normalize_lookup_key(out.get("city") or fallback)
    hint = CITY_REGION_HINTS.get(city_key)
    if hint:
        out["city"] = str(out.get("city") or hint.get("city") or "").strip()
        current_state = str(out.get("state") or "").strip()
        current_country = str(out.get("country") or "").strip()
        if not current_state or normalize_lookup_key(current_state) == city_key:
            out["state"] = str(hint.get("state") or current_state).strip()
        else:
            out["state"] = current_state
        if (not current_country) or len(current_country) <= 3:
            out["country"] = str(hint.get("country") or current_country).strip()
        else:
            out["country"] = current_country

    if not str(out.get("city") or "").strip() and fallback:
        out["city"] = display_label_from_query(fallback)
    return out
...
def location_from_station_name(raw_name, fallback=""):
    ...
    if not raw:
        return enrich_location_guess({"area": "", "city": str(fallback or "").strip(), "state": "", "country": ""}, fallback=fallback)
    ...
    if len(parts) == 1:
        ...
        if fallback_city and normalize_query_text(token).lower() != normalize_query_text(fallback_city).lower():
            return enrich_location_guess({"area": token, "city": fallback_city, "state": "", "country": ""}, fallback=fallback)
        return enrich_location_guess({"area": "", "city": token or fallback_city, "state": "", "country": ""}, fallback=fallback)
    ...
    city = ""
    if fallback_city:
        for p in reversed(middle_parts):
            if normalize_query_text(p).lower() == normalize_query_text(fallback_city).lower():
                city = p
                break

    if not city:
        candidate_parts = parts[1:-1] if len(parts) > 2 else middle_parts
        for token in reversed(candidate_parts):
    ...
    if len(parts) >= 3:
        ...
        nt = normalize_lookup_key(state_candidate)
        if nt and nt != normalize_lookup_key(country):
            state = state_candidate
    if not state and len(parts) >= 4:
        state = parts[-2]
    ...
    return enrich_location_guess({"area": area, "city": city, "state": state, "country": country}, fallback=fallback)
...
def initialize():
    ...
    if csv_path:
        ...
        print("ℹ globalAirQuality.csv not found; live WAQI dashboard still works")
    ...
    print("✓ Live dashboard ready" if WAQI_TOKEN else "⚠ Live dashboard ready, but WAQI is disabled")
    print("="*60)
...
def live_nearby():
    ...
    payload, code = resolve_nearest_live_station(
        lat=lat,
        lng=lng,
        radius_limit=request.args.get("radius_km"),
    )
    if code == 200 and isinstance(payload, dict) and isinstance(payload.get("nearest"), dict):
        nearest = payload["nearest"]
        print(
            f"✓ Nearby station resolved via uid={nearest.get('uid')} "
            f"distance={float(nearest.get('distance_km') or 0.0):.3f} km"
        )
    return jsonify(payload), code
...
def api_status():
    ...
    return jsonify({
        ...
        "location_token_configured": bool(IPINFO_TOKEN),
        "location_token_source": IPINFO_TOKEN_SOURCE,
...
def fetch_ipinfo_location():
    if not IPINFO_TOKEN:
        return None, {"error": "Location API token not configured on server", "code": "location_token_missing"}, 503

    endpoints = [
        ("ipinfo", f"{IPINFO_BASE_URL}/json?token={quote(IPINFO_TOKEN, safe='')}"),
        ("ipinfo-lite", f"https://api.ipinfo.io/lite/me?token={quote(IPINFO_TOKEN, safe='')}"),
    ]
    last_error = None
    for source, url in endpoints:
        try:
            resp = requests.get(
                url,
                headers={"Accept": "application/json", "User-Agent": "AirPulse/1.0"},
                timeout=6,
            )
            if resp.status_code >= 400:
                last_error = {
                    "error": f"Location provider returned HTTP {resp.status_code}",
            ...
            if isinstance(payload, dict):
                loc_text = str(payload.get("loc") or "").strip()
                if loc_text and "," in loc_text:
                    raw_lat, raw_lng = loc_text.split(",", 1)
                    lat = safe_float(raw_lat, None)
                    lng = safe_float(raw_lng, None)
                if lat is None:
                    lat = safe_float(payload.get("latitude"), None)
                if lng is None:
                    lng = safe_float(payload.get("longitude") or payload.get("lng"), None)

            if lat is None or lng is None:
                last_error = {
                    "error": "Location provider did not return coordinates",
                    "code": "location_provider_no_coords",
                    "provider": source,
                }
                continue

            location = enrich_location_guess({
                "city": payload.get("city") or payload.get("city_name") or "",
                "state": payload.get("region") or payload.get("region_name") or "",
                "country": payload.get("country_name") or payload.get("country") or "",
                "latitude": round(float(lat), 6),
                "longitude": round(float(lng), 6),
                "postal": str(payload.get("postal") or "").strip(),
                "timezone": str(payload.get("timezone") or "").strip(),
                "ip": str(payload.get("ip") or "").strip(),
            }, fallback=payload.get("city") or "")
            return location, {"provider": source, "raw": payload}, 200
        except Exception as exc:
            last_error = {
                "error": str(exc),
                "code": "location_provider_exception",
                "provider": source,
            }
            continue

    return None, last_error or {"error": "Unable to resolve device location", "code": "location_provider_unavailable"}, 502


@app.route("/api/location/bootstrap")
def location_bootstrap():
    location, meta, code = fetch_ipinfo_location()
    if code != 200 or not isinstance(location, dict):
        return jsonify({
            "status": "error",
    ...
    out = {
        "status": "ok",
        "source": meta.get("provider") if isinstance(meta, dict) else "ipinfo",
        "location": location,
        "token_configured": bool(IPINFO_TOKEN),
        "token_source": IPINFO_TOKEN_SOURCE,
    }
    if WAQI_TOKEN:
        payload, payload_code = resolve_nearest_live_station(
            lat=location.get("latitude"),
            lng=location.get("longitude"),
            radius_limit=request.args.get("radius_km"),
        )
        if payload_code == 200:
            out["nearest_live"] = payload.get("nearest")
            out["station"] = build_current_aqi_from_live_payload(payload, requested_city=location.get("city") or "")
        else:
            out["nearest_live_error"] = payload.get("data") if isinstance(payload, dict) else "Unavailable"
    return jsonify(out), 200
...
def build_current_aqi_from_live_payload(live_payload, requested_city=""):
    ...
    state_name = str(loc.get("state") or "").strip()
...
def build_live_row_from_payload(live_payload, requested_city=""):
    ...
    return {
        ...
        "state": str(base.get("state") or "").strip(),
...
def build_location_hierarchy(rows):
    tree = {}
    for row in rows or []:
        aqi_val = _to_float_or_none((row or {}).get("aqi"))
        if aqi_val is None:
            continue
        city = str(row.get("city") or row.get("query") or "").strip()
        if not city:
            continue
        area = str(row.get("area") or "").strip()
        state = str(row.get("state") or "").strip()
        country = str(row.get("country") or "").strip()
        hint = CITY_REGION_HINTS.get(normalize_lookup_key(city))
        if hint:
            state = state or hint.get("state") or ""
            country = country or hint.get("country") or ""

        country_name = country or "Other"
        state_name = state or city
        country_bucket = tree.setdefault(country_name, {
            "name": country_name,
            "cities_monitored": 0,
            "max_aqi": 0.0,
            "states": {},
        })
        state_bucket = country_bucket["states"].setdefault(state_name, {
            "name": state_name,
            "cities": [],
        })
        state_bucket["cities"].append({
            "name": city,
            "query": str(row.get("query") or city).strip() or city,
            "area": area,
            "aqi": float(aqi_val),
            "country": country_name,
            "state": state_name,
        })
        country_bucket["cities_monitored"] += 1
        country_bucket["max_aqi"] = max(country_bucket["max_aqi"], float(aqi_val))

    out = []
    for country_name, country_bucket in tree.items():
        states = []
        for state_name, state_bucket in country_bucket["states"].items():
            state_bucket["cities"].sort(key=lambda item: (-float(item.get("aqi") or 0.0), item.get("name") or ""))
            states.append({
                "name": state_name,
                "cities": state_bucket["cities"],
            })
        states.sort(key=lambda item: item["name"])
        out.append({
            "name": country_name,
            "cities_monitored": int(country_bucket["cities_monitored"]),
            "max_aqi": float(country_bucket["max_aqi"]),
            "states": states,
        })
    out.sort(key=lambda item: (-float(item.get("max_aqi") or 0.0), item.get("name") or ""))
    return out
...
def current_aqi():
    try:
        if not WAQI_TOKEN:
            return waqi_token_missing_response()
...
def location_hierarchy():
    try:
        ...
        rows = get_live_snapshot_rows(force=force_fresh)
        if not rows:
            return jsonify({"error": "No live location data available right now"}), 404
        return jsonify({
            "countries": build_location_hierarchy(rows),
            "updated_at": datetime.now().isoformat(),
        })
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@app.route("/api/nlp/advice", methods=["GET", "POST"])
def nlp_advice():
    ...
    try:
        ...
        aqi = safe_float(source.get("aqi"), 0)
        dominant = str(source.get("dominant", "pm25") or "pm25").strip()
        temp = safe_float(source.get("temp"), 0)
        humidity = safe_float(source.get("humidity"), 0)
        wind = safe_float(source.get("wind"), 0)
        ...
        if ai_payload and merge_guidance_payload:
        ...
        return jsonify({"data": advice, "source": response_source})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def city_ranking():
    try:
        ...
        force_fresh = _is_true_query_flag(request.args.get("fresh"))
        country_filter = normalize_lookup_key(request.args.get("country"))
        state_filter = normalize_lookup_key(request.args.get("state"))
        city_filter = normalize_lookup_key(request.args.get("city"))
        latest = get_live_snapshot_rows(force=force_fresh)
        if not latest:
            return jsonify({"error": "No live data"}), 404
        ...
        for r in latest:
            ...
            if aqi_val is None:
                continue
            country_name = str(r.get("country") or "").strip()
            state_name = str(r.get("state") or "").strip()
            city_name = str(r.get("city") or "").strip()
            hint = CITY_REGION_HINTS.get(normalize_lookup_key(city_name))
            if hint:
                country_name = country_name or hint.get("country") or ""
                state_name = state_name or hint.get("state") or ""
            if country_filter and normalize_lookup_key(country_name) != country_filter:
                continue
            if state_filter and normalize_lookup_key(state_name or city_name) != state_filter:
                continue
            if city_filter and normalize_lookup_key(city_name) != city_filter:
                continue
            cat = get_category(int(max(0, round(aqi_val))))
            rows.append({
                "city": city_name,
        ...
        rows.sort(key=lambda r: (-float(r.get("aqi") or 0.0), r.get("city") or ""))
        return jsonify({
            "cities": rows,
            "filters": {
...
def write_alert_subscriptions(subscriptions):
    serializable = subscriptions if isinstance(subscriptions, list) else []
    with SUBSCRIPTIONS_LOCK:
        SUBSCRIPTIONS_FILE.write_text(json.dumps(serializable, indent=2), encoding="utf-8")


def send_alert_email(to_email, subject, text_body):
...
def build_alert_email_text(subscription, row, trigger_reason="threshold"):
    city = str(subscription.get("city") or row.get("city") or subscription.get("query") or "your location").strip()
    state = str(subscription.get("state") or row.get("state") or "").strip()
    country = str(subscription.get("country") or row.get("country") or "").strip()
    parts = [part for part in [city, state, country] if part]
    location_label = ", ".join(parts) if parts else city
    aqi_val = int(round(float(row.get("aqi") or 0.0)))
    category = str(row.get("category") or get_category(max(0, aqi_val)).get("level") or "Unknown")
    threshold = int(round(float(subscription.get("threshold") or 0.0)))
    updated = str(row.get("timestamp") or datetime.now().strftime("%Y-%m-%d %H:%M"))
    precautions = build_nlp_advice(
        city=city,
        country=country,
        aqi=aqi_val,
        dominant=(row.get("pollutants") or {}).get("dominant") or "pm25",
        temp=(row.get("weather") or {}).get("temperature"),
        humidity=(row.get("weather") or {}).get("humidity"),
        wind=(row.get("weather") or {}).get("wind_speed"),
        time_iso=str(row.get("timestamp_iso") or ""),
    )
    steps = precautions.get("action_steps") or precautions.get("precautions") or []
    steps_text = "\n".join(f"- {str(item).strip()}" for item in steps[:5]) or "- Stay aware of local air quality updates."
    if trigger_reason == "manual":
        heading = "Here is your current AQI snapshot."
    else:
        heading = f"Your AQI alert threshold ({threshold}) has been reached."
    return (
        f"AirPulse AQI Alert\n\n"
        f"{heading}\n\n"
...
def _subscription_identity(subscription):
    return (
        str(subscription.get("email") or "").strip().lower(),
        normalize_lookup_key(subscription.get("query") or subscription.get("city") or ""),
    )


def upsert_alert_subscription(subscription):
    subscriptions = read_alert_subscriptions()
    target_key = _subscription_identity(subscription)
    replaced = False
    for idx, existing in enumerate(subscriptions):
        if _subscription_identity(existing) == target_key:
            merged = dict(existing)
            merged.update(subscription)
            subscriptions[idx] = merged
            replaced = True
            break
    if not replaced:
        subscriptions.append(subscription)
    write_alert_subscriptions(subscriptions)
    return subscription


def trigger_alert_delivery(subscription, trigger_reason="threshold"):
    query = str(subscription.get("query") or subscription.get("city") or "").strip()
    if not query:
        raise RuntimeError("Subscription query is missing")
    row = fetch_live_city_row(query, allow_cached_payload=False)
    if not row:
        raise RuntimeError("Live AQI data is unavailable for this subscription")
    aqi_val = _to_float_or_none(row.get("aqi"))
    if aqi_val is None:
        raise RuntimeError("Live AQI is unavailable for this subscription")

    subject = f"AirPulse AQI Alert - {row.get('city') or subscription.get('city') or query}"
    text_body = build_alert_email_text(subscription, row, trigger_reason=trigger_reason)
    send_alert_email(str(subscription.get("email") or "").strip(), subject, text_body)
    return row


def run_alert_scan():
    if not smtp_is_configured() or not WAQI_TOKEN:
        return
    subscriptions = read_alert_subscriptions()
    ...
    for subscription in subscriptions:
        try:
            if not bool(subscription.get("enabled", True)):
                continue
            last_sent_at = str(subscription.get("last_sent_at") or "").strip()
            if last_sent_at:
                try:
                    last_dt = datetime.fromisoformat(last_sent_at.replace("Z", "+00:00")).replace(tzinfo=None)
                    if (now - last_dt).total_seconds() < AQI_ALERT_COOLDOWN_MIN * 60:
                        continue
                except Exception:
                    pass
            ...
            if aqi_val is None:
                continue
            threshold = max(0.0, float(subscription.get("threshold") or 0.0))
            if float(aqi_val) < threshold:
                continue

            subject = f"AirPulse AQI Alert - {row.get('city') or query}"
            text_body = build_alert_email_text(subscription, row, trigger_reason="threshold")
            send_alert_email(str(subscription.get("email") or "").strip(), subject, text_body)
            subscription["last_sent_at"] = now.isoformat()
            subscription["last_aqi"] = float(aqi_val)
            changed = True
        except Exception as exc:
            print(f"[WARN] AQI alert delivery failed for {subscription.get('email')}: {exc}")
            continue
...
def alert_worker_loop():
    while not ALERT_STOP_EVENT.wait(AQI_ALERT_CHECK_INTERVAL_SEC):
        try:
            run_alert_scan()
        except Exception as exc:
            print(f"[WARN] AQI alert worker error: {exc}")


def ensure_alert_worker_started():
    global ALERT_THREAD
    if ALERT_THREAD and ALERT_THREAD.is_alive():
        return
    ALERT_STOP_EVENT.clear()
    ALERT_THREAD = Thread(target=alert_worker_loop, name="aqi-alert-worker", daemon=True)
    ALERT_THREAD.start()
...
def notifications_status():
    return jsonify({
        ...
        "subscriptions": len(read_alert_subscriptions()),
        "cooldown_minutes": AQI_ALERT_COOLDOWN_MIN,
        "check_interval_sec": AQI_ALERT_CHECK_INTERVAL_SEC,
    })
...
def notifications_subscribe():
    try:
        ...
        query = str(payload.get("query") or payload.get("city") or "").strip()
        city = str(payload.get("city") or query).strip()
        state = str(payload.get("state") or "").strip()
        country = str(payload.get("country") or "").strip()
        threshold = max(0, int(round(safe_float(payload.get("threshold"), 120))))
        send_now = bool(payload.get("send_now", True))

        if not smtp_is_configured():
            return jsonify({"error": "SMTP is not configured on this server", "code": "smtp_not_configured"}), 503
        if not re.fullmatch(r"[^@\s]+@[^@\s]+\.[^@\s]+", email):
            return jsonify({"error": "Please enter a valid email address"}), 400
        if not query:
            return jsonify({"error": "Please choose a location before subscribing"}), 400
        ...
        upsert_alert_subscription(subscription)

        sent = False
        row = None
        if send_now:
            row = trigger_alert_delivery(subscription, trigger_reason="manual")
            sent = True
            subscriptions = read_alert_subscriptions()
            for existing in subscriptions:
                if _subscription_identity(existing) == _subscription_identity(subscription):
                    existing["last_sent_at"] = datetime.utcnow().isoformat()
                    existing["last_aqi"] = _to_float_or_none((row or {}).get("aqi"))
        ...
        return jsonify({
            ...
            "current_aqi": build_current_aqi_response_from_row(row) if isinstance(row, dict) else None,
        })
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
if __name__ == "__main__":
    ...
    if smtp_is_configured():
        print(f"📧 AQI email alerts: enabled via {SMTP_HOST}:{SMTP_PORT}")
    else:
        print("📧 AQI email alerts: SMTP not configured")
